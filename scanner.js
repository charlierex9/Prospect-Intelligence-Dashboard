...
const CONFIG = {
  ...
  THRESHOLD_M: 10,
  MODEL:       "claude-sonnet-4-5-20250929",   // Stage 2 formatting
  SCOUT_MODEL: "claude-haiku-4-5-20251001",     // Stage 1 web search
};
...
SEARCH (run many queries):
Contracts Finder / DASA / Innovate UK / UKRI awards ${m} ${y}
Companies House SH01, PSC changes, NED appointments (ex-Goldman/Rothschild/Lazard/PE) defence cyber quantum UK ${y}
UK deep-tech fundraisers ${m} ${y}: quantum, semiconductors, space, cyber, AI, dual-use, OSINT, biosurveillance, counter-drone
M&A and exits ${y} ${ly}: defence tech, cyber, AI, sports clubs, govtech, PE buyouts; check thetakeoverpanel.org.uk
RNS / PDMR director share disposals defence cyber technology ${y}
PE exit window (firms backed 2018-2021) ${y}
IPO / AIM admissions ${y}
Specialist press: Sifted, Jane's, The Engineer, UK Defence Journal
...
async function main(){
  ...
  const raw = await callAnthropic([{role:"user",content:buildScoutPrompt()}], true, 8000, CONFIG.SCOUT_MODEL);
  ...
  const jsonText = await callAnthropic([{role:"user",content:buildFormatterPrompt(raw,today)}], false, 8000, CONFIG.MODEL);
